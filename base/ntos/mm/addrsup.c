...
bool rb_is_red(PMMADDRESS_NODE rb);
bool rb_is_black(PMMADDRESS_NODE rb);
void rb_set_black(PMMADDRESS_NODE rb);
void rb_set_red(PMMADDRESS_NODE rb);
...
void rb_erase_color(PMMADDRESS_NODE node, PMMADDRESS_NODE parent, PMM_AVL_TABLE root);
...
//  parent, siblingwise it returns FALSE.
//
//  BOOLEAN
//  MiIsLeftChild (
//      PRTL_SPLAY_LINKS Links
...

{
    ...
    if (SearchResult == TableEmptyTree) {
        ...
        while(1)
        {
            parent=SANITIZE_PARENT_NODE(NodeToInsert->u1.Parent);
            ...
            {
                PMMADDRESS_NODE uncle = gparent->LeftChild;
				if ( uncle && rb_is_red (uncle))
				{
					rb_set_black(uncle);
					rb_set_black(parent);
...
static void rb_set_parent(PMMADDRESS_NODE rb,
				       PMMADDRESS_NODE p)
{
	rb->u1.Parent = (PMMADDRESS_NODE)((unsigned long)p + rb_color(rb));
}
...
void rb_set_black(PMMADDRESS_NODE rb)
{
    rb->u1.Balance = RB_BLACK;
    return;
}

void rb_set_red(PMMADDRESS_NODE rb)
{
    rb->u1.Balance = RB_RED;
    return;
}
...
void rb_erase_color(PMMADDRESS_NODE node, PMMADDRESS_NODE parent, PMM_AVL_TABLE root)
{
	PMMADDRESS_NODE sibling;
	while (1)
	{
		if(!((!node || rb_is_black(node)) && node != root->BalancedRoot.RightChild))
            break;
        ...
        if (parent->LeftChild == node)
		{
			...
			if ((!sibling->LeftChild || rb_is_black(sibling->LeftChild)) &&
				(!sibling->RightChild || rb_is_black(sibling->RightChild)))
			{
				rb_set_red(sibling);
				node = parent;
			...
			{
				if (!sibling->RightChild || rb_is_black(sibling->RightChild))
				{
					rb_set_red(sibling);
                    rb_set_black(sibling->LeftChild);
					rb_rotate_right(sibling, root);
					sibling = parent->RightChild;
				}
				sibling->u1.Balance = rb_color(parent);
				rb_set_black(parent);
				rb_set_black(sibling->RightChild);
				rb_rotate_left(parent, root);
		...
		else
		{
			...
			else
			{
				...
				sibling->u1.Balance=parent->u1.Balance;
				rb_set_black(parent);
				rb_set_black(sibling->LeftChild);
				rb_rotate_right(parent, root);
				node = root->BalancedRoot.RightChild;
